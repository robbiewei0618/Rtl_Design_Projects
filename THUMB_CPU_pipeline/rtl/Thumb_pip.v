...
module thumb (read_instruction_n, instruction_address,
  ...
  wire [`WORD_SIZE-1:0] PC_w;
  wire [`WORD_SIZE-1:0] LR_w;        // link register = R14
  wire [`WORD_SIZE-1:0] SP_w;        // stack pointer = R13
  wire [`HWORD_SIZE-1:0] IR_w;       // instruction register
  wire [`WORD_SIZE-1:0] R_w [`REG_FILE_SIZE-1:0]; // general regs
  wire [`WORD_SIZE-1:0] IF_IR_w, IF_PC_w;
  wire read_instruction_n_w;
  wire [`WORD_SIZE-1:0] ID_Rd_w, ID_Rn_w, ID_Rm_Rs_w; // Rd, Rn, Rm/Rs
  wire [`WORD_SIZE-1:0] EX_ALU_out_w;// ALU_out at end of EX stage
  wire [5:0] ID_opcode_w, EX_opcode_w; // internal Thumb opcode
  wire [10:0] ID_imm_offset_w; // immediate, offset or register list
  wire [7:0] EX_imm_offset_w;  // 8 bits required in WB stage
  wire [3:0] ID_cond_w;        // ARM condition code (same as THUMB)
  wire [2:0] ID_Rd_code_w, EX_Rd_code_w;  // Rd register number
  ...
  IF_stage if_stage(
      ...
      .read_instruction_n(read_instruction_n),
